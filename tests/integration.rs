...
async fn start_echo_server(addr: &str) -> Result<u16> {
...
#[tokio::test]
async fn test_roundtrip_tcp_ws_tcp() -> Result<()> {
    ...
    let mut client = TcpStream::connect(format!("127.0.0.1:{}", client_port)).await?;
...
#[tokio::test]
async fn test_multiple_routes() -> Result<()> {
    ...
    let client1_listener = TcpListener::bind("127.0.0.1:0").await?;
    let client1_port = client1_listener.local_addr()?.port();
    ...
    let client2_listener = TcpListener::bind("127.0.0.1:0").await?;
    let client2_port = client2_listener.local_addr()?.port();
    ...
    tokio::spawn(async move {
        proxy_client1.run().await?;
        Ok::<_, anyhow::Error>(())
    });

    tokio::spawn(async move {
        proxy_client2.run().await?;
        Ok::<_, anyhow::Error>(())
    });
    ...
    let mut client1 = TcpStream::connect(format!("127.0.0.1:{}", client1_port)).await?;
    let mut client2 = TcpStream::connect(format!("127.0.0.1:{}", client2_port)).await?;

    let msg1 = b"Route 1 message";
    let msg2 = b"Route 2 message";

    client1.write_all(msg1).await?;
    client2.write_all(msg2).await?;

    let mut response1 = vec![0u8; msg1.len()];
    let mut response2 = vec![0u8; msg2.len()];

    client1.read_exact(&mut response1).await?;
    client2.read_exact(&mut response2).await?;
...
fn generate_test_certs() -> Result<(String, String, String)> {
...
#[tokio::test]
async fn test_wss_with_custom_ca() -> Result<()> {
    ...
    std::fs::File::create(&ca_path)?.write_all(ca_pem.as_bytes())?;
    ...
    let backend_server = TcpListener::bind("127.0.0.1:0").await?;
    let backend_port = backend_server.local_addr()?.port();
    ...
    let tls_options = TlsOptions {
        insecure: false,
        ca_cert_path: Some(ca_path.to_str().context("invalid ca path")?.to_string()),
    };
...
#[tokio::test]
async fn test_wss_insecure() -> Result<()> {
    let temp_dir = tempfile::tempdir()?;
    ...
    client_conn.write_all(test_msg).await?;

    let mut received = vec![0u8; test_msg.len()];
    backend_conn.read_exact(&mut received).await?;
    assert_eq!(received, test_msg);

    // Response
    let response_msg = b"Insecure WSS response";
    backend_conn.write_all(response_msg).await?;

    let mut received = vec![0u8; response_msg.len()];
    client_conn.read_exact(&mut received).await?;
...
#[tokio::test]
async fn test_wss_self_signed_server() -> Result<()> {
    // 1. Start a TCP listener (the backend server)
    let backend_server = TcpListener::bind("127.0.0.1:0").await?;
    let backend_port = backend_server.local_addr()?.port();
...
"#,
    ...
    File::create(&config_path)?.write_all(initial_config.as_bytes())?;

    // Start server with config
    let config_path_clone = config_path.clone();
    tokio::spawn(async move {
        run_with_config(&config_path_clone).await?;
        Ok::<_, anyhow::Error>(())
    });
...
"#,
    ...
    let mut client2 = TcpStream::connect(format!("127.0.0.1:{}", client_port)).await?;
    let test_msg2 = b"test2";
    client2.write_all(test_msg2).await?;
    let mut received2 = vec![0u8; test_msg2.len()];
    client2.read_exact(&mut received2).await?;
...
#[tokio::test]
async fn test_hot_reload_preserves_connections() -> Result<()> {
    use std::fs::File;
...
"#,
    ...
    let mut active_conn = TcpStream::connect(format!("127.0.0.1:{}", client_port)).await?;

    // Verify it works initially
    let msg1 = b"before_reload";
    active_conn.write_all(msg1).await?;
    let mut received = vec![0u8; msg1.len()];
    active_conn.read_exact(&mut received).await?;
...
"#,
    ...
    active_conn.write_all(msg2).await?;
    let mut received2 = vec![0u8; msg2.len()];
    active_conn.read_exact(&mut received2).await?;
    ...
    for i in 0..5 {
        ...
        active_conn.write_all(msg.as_bytes()).await?;
